...
app.config['MYSQL_DB'] = 'real_time_messaging'
...
def chatting(id):
    if 'uid' in session:
        ...
        if get_result > 0:
            ...
            # Close Connection
            cur.close()
            return render_template('chat_room.html', users=users, form=form, session=session)
        else:
...
def chats():
    if 'lid' in session:
        ...
        # Close Connection
        cur.close()
        return render_template('chats.html', chats=chats,session=session)
    return redirect(url_for('login'))
